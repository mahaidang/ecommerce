...
const CustomerProductDetail = () => {
    // Lấy userId và sessionId
    // import { getUserIdFromToken, getOrCreateSessionId } from "@/lib/session";
    const userId = getUserIdFromToken();
    const sessionId = getOrCreateSessionId();
    // Import hook lưu item vào giỏ hàng
    // import { useSaveItem } from "@/features/basket/hooks"; // Đảm bảo đã import đúng
    const { mutate: saveItem, isSuccess } = useSaveItem();
                                    ...
                                    <div className="flex gap-3">
                                        {visibleImages?.map((img, i) => (
                                            <button
                                                ...
                                                className={`w-20 h-20 rounded-xl overflow-hidden border-2 transition-all hover:scale-105 hover:shadow-lg ${mainImage === img.url
                                ...
                                <div className="space-y-6">
                                    <div>
                                        ...
                                        <div className="flex items-center gap-3 mb-4">
                                            ...
                                            <span className={`px-3 py-1 rounded-full text-xs font-semibold ${p.isActive
